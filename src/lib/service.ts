...
export async function fetchDetailService(slug: string): Promise<ServiceDetail | null> {
  const url = `${STRAPI_URL}/services?filters[slug][$eq]=${slug}` +
...
export async function fetchAllServices(): Promise<ServiceDetail[]> {
  ...
  const url = `${STRAPI_URL}/services?populate[0]=background_image_before&populate[1]=background_image_after&populate[2]=background_image&populate[3]=seo`;
  const res = await fetch(url);
  if (!res.ok) return [];
